...
export default function Favourites() {
  ...
  function handleRemoveAll(event: React.MouseEvent<HTMLButtonElement>) {
    Object.keys(localStorage).forEach(key => {
      if (!key.endsWith('_audio') && !key.endsWith('_ended')) {
  ...
  return (
    <div className="fav-episodes-container">
      {loading ? (
        <LoadIcon />
      ) : (
        <div className="fav-episodes-content">
          <div className="fav-episodes-header">
            <h1>Favourite Episodes</h1>
            <button onClick={handleRemoveAll}>Remove All</button>
            <div className="fav-episodes-sort">
              <button onClick={handleSort}>A-Z</button>
              <button onClick={handleSort}>Z-A</button>
              <button onClick={handleSort}>Newest</button>
              <button onClick={handleSort}>Oldest</button>
            </div>
          </div>
          {renderedShows}
        </div>
